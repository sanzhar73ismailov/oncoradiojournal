...
{/foreach}
<p/>
<b>Түр:</b> {$publication->section_name}
<p/>
<b>УДК:</b> {$publication->code_udk}
<p/>
<b>Жыл:</b> {$issue->year} <b>шығарылым:</b> {$issue->issue}  <b>нөмірі:</b> {$issue->number} <b>бет:</b> {$publication->p_first}-{$publication->p_last}
<p/>
...
<p/>
<b>Түйінді сөздер:</b> {$publication->keywords}
<p/>
<b>Сілтеме :</b>  
{foreach from=$authors item=author name=authors_cycle}
