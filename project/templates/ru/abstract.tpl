...
{/foreach}
<p/>
<b>Тип:</b> {$publication->section_name}
<p/>
<b>УДК:</b> {$publication->code_udk}
<p/>
<b>Год:</b> {$issue->year} <b>выпуск:</b> {$issue->issue}  <b>номер:</b> {$issue->number} <b>страницы:</b> {$publication->p_first}-{$publication->p_last}
<p/>
...
<p/>
<b>Ключевые слова:</b> {$publication->keywords}
<p/>
<b>Ссылка:</b>  
{foreach from=$authors item=author name=authors_cycle}
